...
(function(factory) {
    ...
    // Set up ModalFlow appropriately for the environment. Start with AMD.
    if (typeof define === 'function' && define.amd) {
        define(['jquery', 'exports'], function($, exports) {
            // Export global even in AMD case in case this script is loaded with
            // others that may still expect a global Backbone.
            root.ModalFlow = factory(root, exports, $);
        });
        ...
        root.ModalFlow = factory(root, {}, (root.jQuery || root.Zepto || root.ender || root.$));
    }
}(function(root, ModalFlow, $) {
    ...
    //BUILD MODAL
        /*
         ...
         clickthroughDataUpdater: If using clickthroughLink, this optional function mixes data
             into input to pass along
    */
    ...
    function Modal(options) {
        ...
        this.clickthroughDataUpdater = options.clickthroughDataUpdater || null;

        if (options.ajaxParameters) {
            this.submit = this.configureAjax(options.ajaxParameters);
        } else {
            this.submit = function () {
                var data = this.clickthroughDataUpdater ? this.clickthroughDataUpdater(this.input) : this.input;
    ...
    ModalFlow.buildModalWith = function(options) {
    ...
    ModalFlow.chain = function() {
        ...
        if (typeof nextLink === "function") {
            from = steps.pop();
            nextLink = this.linkExit(from, nextLink);
        }

        //Build the links, starting from the end of the array
        while (steps.length > 0) {
            from = steps.pop();
            nextLink = this.link(from, nextLink);
        }
    ...
    ModalFlow.link = function(lastModal, nextModalOrTransition) {
    ...
    ModalFlow.linkBranches = function(lastModal, branchingFunction) {
        return new Transition(lastModal, branchingFunction);
    };

    //For linking to a function that exits the modal workflow
    ModalFlow.linkExit = function(lastModal, exitFunction) {
